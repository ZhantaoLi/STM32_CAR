...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	unsigned char more;
	long quat[4]; 
  //从FIFO读取新数据，储存了陀螺仪、加速度、四元数及手势数据
	//传感器数据sensors可以告知调用者哪种数据有更新
	...
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		q3 = quat[3]/q30; 
		//计算得到俯仰角/横滚角/航向角
		*pitch = asin(-2*q1*q3+2*q0*q2)*57.3*100;
		*roll  = atan2(2*q2*q3+2*q0*q1,-2*q1*q1-2*q2*q2+1)*57.3*100;
		*yaw   = atan2(2*(q1*q2+q0*q3),q0*q0+q1*q1-q2*q2-q3*q3)*57.3*100;
	}else return 2;
	return 0;
}
